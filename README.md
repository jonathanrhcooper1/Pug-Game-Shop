...
POS/payment repository staging and execution-gate metadata now report deferred
POS/payment log write results, explicit block reasons, zero affected rows, and
transaction-adapter deferral while repository execution, provider capture, and
inventory writes remain disabled.
...
Planned POS/payment route contracts now describe disabled-by-default webhook,
event ingestion, reconciliation, conflict, and fee-snapshot endpoints for later
staging wiring.
POS/payment route readiness now surfaces those planned routes in health/admin
status with explicit blocked reasons for feature gating, missing handlers,
permission callbacks, transaction wiring, webhook verification, and provider
capture/inventory/gateway deferrals while keeping all live routes disabled.
POS/payment permission callbacks now have a fail-closed factory for
manager/system POS capabilities, conflict-resolution/settings capabilities, and
signed provider webhooks; `manage_pos` is manager/system only and live route
registration remains disabled.
A fail-closed POS/payment controller now exposes every planned route callback
for future injection tests, returning disabled responses with route/write,
transaction, webhook, provider capture, provider inventory, and gateway
deferrals intact by default.
POS/payment route registration planning now mirrors the offline route safety
pattern: default plans remain locked, permission/controller readiness is
visible, read routes require cleared registration deferrals, write routes also
require cleared route-connected write deferrals, and webhook routes require
cleared webhook-registration deferrals.
A guarded POS/payment route registrar can now consume those enabled plans and
call WordPress route registration only for future routes that pass every
planner gate; current defaults still register zero POS/payment routes.
...
The WordPress POS/payment route bootstrap hook now uses the dependency factory,
so future staged route registration checks share the parser-only controller and
permission factory while current defaults still expose zero POS/payment routes.
POS/payment fee snapshot listing now has a staged query planner that normalizes
provider, channel, currency, effective-date, and page-size filters for future
admin review reads while read execution, writes, capture, inventory updates,
gateway capture, and route registration remain disabled.
...
remain disabled.
POS/payment fee snapshot repository readiness now appears in parser-only route
metadata and dependency health/admin status, so staging can see whether an
explicit repository adapter was injected without calling it from default route
callbacks.
An explicit staged POS/payment fee snapshot route handler can now be injected
for controlled repository-backed reads, returning normalized fee rows and audit
metadata while keeping the default route factory parser-only and unregistered.
That route handler can now be assembled through an explicit fee snapshot
handler factory and injected into the POS/payment dependency factory for
repository-backed staging tests; defaults still keep route-connected reads
deferred, parser-only, and unregistered.
POS/payment route planning now tracks route-connected read deferral separately,
so future GET routes stay blocked until read execution is explicitly cleared
in addition to handler, permission, and registration readiness.
POS/payment dependency health and admin status now surface route-connected read
deferral alongside route, write, repository, handler, registrar, and
bootstrapper readiness, keeping staging review visible without enabling live
routes.
...
Explicitly enabled staged push route processing now includes that canonical
mutation planning metadata in response/audit payloads and skips replayed
duplicate operations before future canonical write planning.

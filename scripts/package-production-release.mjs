...
function writeLanServerStartupFiles(targetDir, serverZipName) {
  ...
  writeFileSync(
    ...
    [
      ...
      "}",
      "$BundledNode = Join-Path $env:LOCALAPPDATA 'OpenAI\\Codex\\runtimes\\cua_node\\1b23c930bdf84ed6\\bin\\node.exe'",
      "$NodeCommand = Get-Command node -ErrorAction SilentlyContinue",
      "$NodePath = if ($NodeCommand) { $NodeCommand.Source } else { $null }",
      "if (!$NodePath -and (Test-Path -LiteralPath $BundledNode)) { $NodePath = $BundledNode }",
      "if (!$NodePath) { throw 'Node.js 22.13+ or Node.js 24+ is required for the Pug LAN server.' }",
      ...
      "& $NodePath apps/local-sync-server/src/cli.mjs",
      "",
    ].join("\r\n"),
  )

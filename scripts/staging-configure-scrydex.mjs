...
const remoteRunnerPath = `${remoteUploadDir}/scrydex-configure-${timestampForRemoteName(
  new Date(),
)}.php`
...
const providerPayload = {
  ...
  environment: safeChoice(process.env.SCRYDEX_ENVIRONMENT, ["staging", "sandbox"], "staging"),
...
if (dryRun) {
  console.log(
    JSON.stringify(
      {
        action: "staging_scrydex_configure_dry_run",
        statusOnly,
        remoteRunnerPath,
...
if (missingEnv.length > 0) {
  throw new Error(`Missing staging ScryDex environment variables: ${missingEnv.join(", ")}`)
}

if (!statusOnly && process.env.PUG_STAGING_CONFIRM_SCRYDEX_CONFIG !== "configure-staging-scrydex") {
  throw new Error(
    "Set PUG_STAGING_CONFIRM_SCRYDEX_CONFIG=configure-staging-scrydex to update staging ScryDex settings.",
  )
}
...
$request = new WP_REST_Request('GET', '/tcg-store/v1/health');
$admins = get_users(array('role' => 'administrator', 'number' => 1, 'fields' => 'ids'));
wp_set_current_user((int) ($admins[0] ?? 0));
$health = rest_do_request($request);
$health_data = $health->get_data();
echo wp_json_encode(array(
	'action' => 'staging_scrydex_configured',
	'status' => 'ok',
...
const result = await withStagingConnection(async (connection) => {
  ...
  try {
    ...
    return {
      action: statusOnly ? "staging_scrydex_status_checked" : "staging_scrydex_configured",

...
docs.set("release-package/INSTALLATION_AND_DEPLOYMENT_GUIDE.md", header(
  ...
  table(["Component", "Requirement"],
    [
      ...
      ["Node.js/npm", "Required for build, packaging, local sync server, and frontend/app tests. LAN server runtime requires Node 22.13+ or Node 24 with built-in `node:sqlite`."],
      ["Windows app", "Windows workstation for Pug Store App installer; WebView/runtime and network access to LAN server."],
  ...
  steps([
    ...
    "Run `npm.cmd run package:production-release` to build the three-deliverable package.",
  ...
  steps([
    ...
    "Let apps auto-discover the middleman; if blocked, enter the LAN server URL manually, for example `http://SERVER-IP:8787`.",
...
) + section("LAN Server SQLite",
  "The packaged LAN server does not include an existing SQLite database file and does not install SQLite separately. It uses Node's built-in `node:sqlite` module. On startup, the server creates or reuses `store-sync.sqlite`; set `LOCAL_SYNC_SQLITE_PATH` or `PUG_LOCAL_SYNC_DB` to choose another durable database path.",

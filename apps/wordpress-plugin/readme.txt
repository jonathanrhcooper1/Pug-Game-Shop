...
Phase 8.20 wires POS/payment bootstrap through the staged dependency factory
while keeping route registration disabled by default.
Phase 8.21 adds staged POS/payment fee snapshot query planning while keeping
read execution, writes, capture, inventory updates, and route registration
disabled.
Phase 8.22 adds POS/payment fee snapshot SQL-template planning while keeping
repository execution, route registration, writes, capture, and gateway capture
disabled.
Phase 8.23 adds explicit POS/payment fee snapshot repository adaptation for
staged `$wpdb` read tests while keeping route-connected reads, writes, capture,
inventory updates, gateway capture, and route registration disabled.
Phase 8.24 exposes POS/payment fee snapshot repository readiness in parser-only
route validation and dependency health/admin status while keeping default route
callbacks from calling the repository.
Phase 8.25 adds an explicit staged POS/payment fee snapshot route handler for
repository-backed read tests while keeping the default route factory
parser-only and unregistered.
Phase 8.26 adds staged POS/payment fee snapshot route handler factory
composition for explicitly enabled repository-backed read tests while keeping
default dependency wiring parser-only, deferred, and unregistered.
Phase 8.27 adds POS/payment route-connected read deferral gates to route
contracts, registration planning, readiness planning, and bootstrap summaries
so future GET routes cannot register until read execution is explicitly
cleared.
Phase 8.28 exposes POS/payment route-connected read deferral and read-ready
state in dependency health/admin status while keeping default route execution
deferred.
...
* Exposed POS/payment route-connected read deferral and read-ready state in
  dependency health payloads.
* Updated admin status summaries to render route, read, and write gate states
  from dependency payloads instead of static text.
* Kept default route registration, default route-connected reads,
  fee-snapshot writes, provider capture, provider inventory writes, and
  WooCommerce gateway capture disabled.

= 0.153.0 =

* Added POS/payment route-connected read deferral metadata to route contracts,
  registration plans, readiness plans, and bootstrap summaries.
* Added coverage proving future GET routes stay blocked while read execution is
  deferred and can register only after the read gate is explicitly cleared.
* Kept default route registration, default route-connected reads,
  fee-snapshot writes, provider capture, provider inventory writes, and
  WooCommerce gateway capture disabled.
...
* Added staged POS/payment fee snapshot route handler factory composition for
  explicitly enabled repository-backed read tests.
* Added dependency-factory injection coverage for the fee snapshot handler
  factory and admin status metadata for deferred handler readiness.
* Kept default route registration, default route-connected reads,
  fee-snapshot writes, provider capture, provider inventory writes, and
  WooCommerce gateway capture disabled.
...
* Added coverage for successful handler reads, invalid query rejection before
...
* Added POS/payment fee snapshot repository readiness metadata to parser-only
  route validation and dependency health/admin status.
* Added staged coverage proving an injected repository adapter is reported but
  not called by default route callbacks.
* Kept route-connected reads, fee-snapshot writes, route registration,
  provider capture, provider inventory writes, and WooCommerce gateway capture
  disabled.

= 0.149.0 =

* Added explicit POS/payment fee snapshot repository adaptation for staged
  `$wpdb` read tests.
* Added row normalization, table-prefix mismatch protection, database failure
  auditing, and malformed row rejection for fee snapshot reads.
* Kept route-connected reads, fee-snapshot writes, route registration,
  provider capture, provider inventory writes, and WooCommerce gateway capture
  disabled.
...
* Added POS/payment fee snapshot SQL-template planning for staged admin review
  reads.
* Added parser-only route metadata for SQL readiness and prepare-argument
  counts.
* Kept fee snapshot repository execution, route registration, writes, provider
...
* Normalized provider, channel, currency, effective-date, and page-size filters
  in the parser-only fee snapshot route handler.
* Kept current POS/payment route registration, read execution, provider
  capture, provider inventory writes, and WooCommerce gateway capture disabled.

= 0.146.0 =

* Wired the WordPress POS/payment route bootstrapper through the staged
  dependency factory.
* Added unit coverage proving dependency-backed bootstrap can register a
  future explicitly enabled read route in tests while defaults register none.

...
$assert( '0.7.0' === Version::PLUGIN, 'Unexpected plugin version.' );
$assert( 4 === Version::DATABASE, 'Unexpected database target version.' );
$assert( 4 === (int) get_option( MigrationRunner::VERSION_OPTION, 0 ), 'Database version option was not updated.' );
...
$assert( '0.7.0' === ( $data['version'] ?? null ), 'Health response reported the wrong plugin version.' );
$assert( 4 === (int) ( $data['database']['current'] ?? 0 ), 'Health response reported the wrong current schema.' );
$assert( 4 === (int) ( $data['database']['target'] ?? 0 ), 'Health response reported the wrong target schema.' );

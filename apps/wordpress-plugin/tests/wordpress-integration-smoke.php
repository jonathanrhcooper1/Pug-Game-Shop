...
$assert( '0.144.0' === Version::PLUGIN, 'Unexpected plugin version.' );
...
$assert( '0.144.0' === ( $data['version'] ?? null ), 'Health response reported the wrong plugin version.' );
...
$assert( 0 === (int) ( $data['pos_payment_route_dependencies']['controller_handler_count'] ?? -1 ), 'POS/payment route handlers should remain unconfigured by default.' );
$assert( false === ( $data['pos_payment_route_dependencies']['controller_handlers_configured'] ?? null ), 'POS/payment controller handlers should remain unconfigured by default.' );

...
namespace TCGStorePlatform\Tests\Unit {
	...
	final class InventoryIntakeRepositoryTest extends TestCase {
		public function test_repository_inserts_prepared_inventory_intake_plan(): void {
			...
			$this->assert_same( 1, $result->price_change_log_rows_affected() );
			...
			$this->assert_contains( '`public_id`', $database->prepare_queries[1] );
			$this->assert_contains( '`row_version`', $database->prepare_queries[1] );
			$this->assert_contains( 'INSERT INTO `wp_tcg_price_change_log`', $database->prepare_queries[2] );
			$this->assert_contains( '`new_sale_price`', $database->prepare_queries[2] );
			...
			$this->assert_true( $result->response_payload()['price_change_log_persisted'] );
			$this->assert_same( 1, $result->response_payload()['price_change_log_row_count'] );
			$this->assert_same( 'inventory_intake_repository_insert', $audit['action'] );
			$this->assert_same( 707, $audit['insert_id'] );
			$this->assert_true( $audit['price_change_log_persisted'] );
			$this->assert_same( 1, $audit['price_change_log_rows_affected'] );
			...
			$this->assert_false( $audit['price_change_log_execution_deferred'] );
		...
		public function test_repository_rejects_failed_and_unexpected_insert_counts(): void {
			...
			$this->assert_true( $zero_result->audit_payload()['transaction_rolled_back'] );
			...
			$this->assert_true( $two_result->audit_payload()['transaction_rolled_back'] );
		}

		public function test_repository_rolls_back_when_price_change_log_insert_fails(): void {
			$database = new \InventoryIntakeRepositoryWpdb( array( 1, 1, false, 1 ) );
			$result   = ( new InventoryIntakeRepository( $database ) )->create( $this->valid_plan() );
			$audit    = $result->audit_payload();

			$this->assert_true( $result->is_rejected() );
			$this->assert_same( 1, $result->rows_affected() );
			$this->assert_same( 0, $result->price_change_log_rows_affected() );
			$this->assert_same( array( 'inventory_price_change_log_insert_failed' ), $result->errors() );
			$this->assert_same( 3, $database->prepare_count );
			$this->assert_same( 4, $database->query_count );
			$this->assert_contains( 'INSERT INTO `wp_tcg_price_change_log`', $database->prepare_queries[2] );
			...
			$this->assert_false( $audit['price_change_log_persisted'] );
		}

...
final class ScryDexPersistenceRepositoryTest extends TestCase {
	public function test_repository_stages_persistence_queries_as_deferred(): void {
		...
		$this->assert_same( 2, $result->price_point_query_count() );
		$this->assert_same( 1, $result->checkpoint_query_count() );
		$this->assert_same( 9, $result->total_query_count() );
		...
		$this->assert_same( 'provider_price_point_insert', $result->price_point_results()[0]['query_kind'] );
		...
		$this->assert_true( $audit['provider_price_point_writes_deferred'] );
	...
	public function test_repository_executes_valid_query_plan_in_transaction(): void {
		...
		$this->assert_same( 9, $result->rows_affected() );
		$this->assert_same( 2, $result->reference_insert_query_count() );
		$this->assert_same( 2, $result->reference_variant_upsert_query_count() );
		$this->assert_same( 2, $result->price_observation_query_count() );
		$this->assert_same( 2, $result->price_point_query_count() );
		...
		$this->assert_contains( 'INSERT INTO `wp_tcg_provider_price_points`', $database->prepare_queries[6] );
		$this->assert_contains( 'INSERT INTO `wp_tcg_sync_checkpoints`', $database->prepare_queries[8] );
		...
		$this->assert_false( $audit['provider_price_point_writes_deferred'] );

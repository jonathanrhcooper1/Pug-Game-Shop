<?php
/**
 * POS/payment log repository execution gate result.
 *
...
final class PosPaymentLogRepositoryExecutionResult {
	...
	public static function ready( PosPaymentLogRepositoryResult $repository_result ): self {
		return new self(
			self::STATUS_READY,
			array(),
			array(),
	...
	public function pos_sync_query_count(): int {
		return $this->non_negative_int( $this->repository_audit['pos_sync_query_count'] ?? 0 );
	}

	public function payment_provider_query_count(): int {
		return $this->non_negative_int( $this->repository_audit['payment_provider_query_count'] ?? 0 );
	}

	public function total_query_count(): int {
		return $this->non_negative_int( $this->repository_audit['total_query_count'] ?? 0 );
	}
	...
	public function log_idempotency_keys(): array {
		return $this->string_list( $this->repository_audit['log_idempotency_keys'] ?? array() );
	}

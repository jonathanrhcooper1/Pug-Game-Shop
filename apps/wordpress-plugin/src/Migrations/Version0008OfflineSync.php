...
final class Version0008OfflineSync implements Migration {
	...
	public function up( \wpdb $database ): void {
		...
		$tables = OfflineSyncSchema::tables(
			$database->prefix,
	...
	public function down( \wpdb $database ): void {
		foreach ( OfflineSyncSchema::drop_order( $database->prefix ) as $table_name ) {
	...
	public function checksum(): string {
		return hash(
			...
			implode(
				"\n",
				OfflineSyncSchema::tables( 'wp_', 'DEFAULT CHARACTER SET utf8mb4' )
			)
		);
	}
}

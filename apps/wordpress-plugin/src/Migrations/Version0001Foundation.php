...
final class Version0001Foundation implements Migration {
	...
	public function up( \wpdb $database ): void {
		...
		$tables = FoundationSchema::tables(
			$database->prefix,
	...
	public function down( \wpdb $database ): void {
		foreach ( FoundationSchema::drop_order( $database->prefix ) as $table_name ) {
	...
	public function checksum(): string {
		return hash(
			...
			implode(
				"\n",
				FoundationSchema::tables( 'wp_', 'DEFAULT CHARACTER SET utf8mb4' )
			)
		);
	}
}

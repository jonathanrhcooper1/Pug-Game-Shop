...
final class AdminMenu {
	...
	public function handle_square_mapping_update(): void {
		if ( ! current_user_can( 'edit_inventory' ) ) {
			wp_die( esc_html__( 'You do not have permission to update inventory mappings.', 'tcg-store-platform' ) );
		}

		check_admin_referer( 'tcg_store_square_mapping_update', 'tcg_store_square_mapping_nonce' );
		...
		$status              = true === ( $result['synced'] ?? false ) ? 'square_mapping_saved' : 'square_mapping_failed';
			...
			add_query_arg(
				array_filter(
					array(
						...
						'tcg_square_mapping_status' => $status,
						'tcg_square_mapping_error'  => $error,
					),
...
@media (max-width:782px){.tcg-report-filters{grid-template-columns:1fr}.tcg-report-chart-bars{height:110px}}
	...
	private function inventory_lookup_query(): array {
		...
		return array(
			...
			'page_size' => $source['lookup_page_size'] ?? 250,
		);
	}
	...
	private function render_square_mapping_panel( array $panel ): void {
		...
		$this->render_square_mapping_update_notice();
		...
		echo esc_attr( wp_create_nonce( 'tcg_store_square_mapping_update' ) );
		echo '">';
		echo '<p>';
	...
	private function render_square_mapping_update_notice(): void {
		$status = sanitize_key( wp_unslash( (string) ( $_GET['tcg_square_mapping_status'] ?? '' ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

		if ( '' === $status ) {
			return;
		}

		if ( 'square_mapping_saved' === $status ) {
			echo '<div class="notice notice-success is-dismissible"><p>';
			echo esc_html__( 'Square POS mapping saved for the selected inventory row.', 'tcg-store-platform' );
			echo '</p></div>';
			return;
		}

		$error = sanitize_text_field( wp_unslash( (string) ( $_GET['tcg_square_mapping_error'] ?? '' ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		echo '<div class="notice notice-error is-dismissible"><p>';
		echo esc_html__( 'Square POS mapping could not be saved.', 'tcg-store-platform' );
		if ( '' !== $error ) {
			echo ' <code>' . esc_html( $error ) . '</code>';
		}
		echo '</p></div>';
	...
	private function render_inventory_lookup_panel( array $panel ): void {
		...
		foreach ( $page_sizes as $page_size ) {
			$page_size = (int) $page_size;
			echo '<option value="' . esc_attr( (string) $page_size ) . '" ' . selected( (int) ( $query['page_size'] ?? 250 ), $page_size, false ) . '>';
	...
	private function render_inventory_lookup_script(): void {
		...
		echo 'const matchingPricePoint=function(card,variant,condition,rawOrGraded){const points=pricePoints(card);const wantedType=String(rawOrGraded||"raw").toLowerCase()==="graded"?"graded":"raw";const variantId=String((variant||{}).provider_variant_id||"");const referenceVariantId=String((variant||{}).reference_variant_id||"");const wantedCondition=String(condition||"").toUpperCase();const typed=points.filter(function(point){const type=String(point.raw_or_graded||"raw").toLowerCase();return wantedType==="graded"?type==="graded":type!=="graded";});const candidates=typed.length?typed:points;return candidates.find(function(point){const pointVariant=String(point.provider_variant_id||"");const pointReferenceVariant=String(point.reference_variant_id||"");const pointCondition=String(point.condition_code||"").toUpperCase();const variantMatches=!variantId&&!referenceVariantId&&!pointVariant&&!pointReferenceVariant||variantId&&pointVariant===variantId||referenceVariantId&&pointReferenceVariant===referenceVariantId;const conditionMatches=!wantedCondition||!pointCondition||pointCondition===wantedCondition;return variantMatches&&conditionMatches;})||candidates.find(function(point){return String(point.condition_code||"").toUpperCase()===wantedCondition;})||candidates[0]||null;};';
...
const parseJsonResponse=function(response){return response.text().then(function(text){let payload=null;try{payload=text?JSON.parse(text):{};}catch(error){const snippet=String(text||"").replace(/\s+/g," ").slice(0,260);return {ok:false,httpStatus:response.status,statusText:response.statusText||"",payload:{error:{code:"non_json_response",message:snippet||"Server returned a non-JSON response."}}};}return {ok:response.ok,httpStatus:response.status,statusText:response.statusText||"",payload:payload};});};
const restErrorMessage=function(result,fallback){const payload=(result||{}).payload||{};const error=payload.error||{};const details=error.details||{};const pieces=[];if(result&&result.httpStatus){pieces.push("HTTP "+result.httpStatus);}if(error.code){pieces.push(error.code);}if(error.message){pieces.push(error.message);}if(Array.isArray(details.errors)&&details.errors.length){pieces.push(details.errors.join(", "));}return pieces.length?pieces.join(" - "):(fallback||"scrydex_catalog_request_failed");};
const postIndex=function(payload){return fetch(resultBox.dataset.endpoint,{method:"POST",headers:{"Content-Type":"application/json","X-WP-Nonce":resultBox.dataset.nonce},body:JSON.stringify(payload)}).then(parseJsonResponse).then(function(result){if(result.ok){return result.payload;}throw new Error(restErrorMessage(result,"scrydex_catalog_request_failed"));}).catch(function(error){if(error&&error.message){throw error;}throw new Error(String(error||"scrydex_catalog_network_failed"));});};
...
const requestText=function(meta){meta=meta||{};const request=meta.request||{};return [request.method,request.path].map(function(value){return String(value||"").trim();}).filter(Boolean).join(" ");};
const providerResultText=function(result){result=result||{};const parts=[];if(result.http_status||result.provider_result_http_status){parts.push("HTTP "+String(result.http_status||result.provider_result_http_status));}if(result.error_code||result.provider_result_error_code){parts.push(String(result.error_code||result.provider_result_error_code));}if(result.message||result.provider_result_message){parts.push(String(result.message||result.provider_result_message));}const meta=result.provider_result_meta||result.provider_result||{};const req=requestText(meta);if(req){parts.push(req);}if(meta.response_message){parts.push(String(meta.response_message));}if(meta.body_excerpt){parts.push("Body: "+String(meta.body_excerpt));}return parts.filter(Boolean).join(" | ");};
const batchIssues=function(batch){batch=batch||{};const issues=[];const add=function(value){value=String(value||"").trim();if(value&&!issues.includes(value)){issues.push(value);}};listValues(batch.block_reasons).forEach(add);listValues(batch.configuration_issues).forEach(add);if(batch.error_code){add(String(batch.error_code));}if(batch.message){add(String(batch.message));}add(providerResultText(batch.provider_result||{}));const pages=Array.isArray(batch.pages)?batch.pages:[];pages.forEach(function(page){const plan=page.orchestration_plan||{};listValues(plan.block_reasons).forEach(add);listValues(plan.configuration_issues).forEach(add);add(providerResultText(page));add(providerResultText(plan));if(plan.persistence_repository_result&&Array.isArray(plan.persistence_repository_result.errors)){listValues(plan.persistence_repository_result.errors).forEach(add);}});return issues;};
...
async function runGameIndex(state,states,indexExpansions,manual){
  ...
  if(!expansionIds.length){
    state.latest="No expansions returned for "+state.game+"; using the whole-game card search endpoint.";
    setProgress(states);
    await indexCardsForExpansion(state,states,"");
  }else{

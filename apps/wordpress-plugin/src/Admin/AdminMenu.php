...
final class AdminMenu {
	...
	private function render_inventory_lookup_script(): void {
		...
		echo 'const priceLabel=function(card){const price=card.market_price||{};const amount=price.amount||card.market_price_amount||"";const currency=card.currency||price.currency||"USD";const pointLabels=pricePoints(card).slice(0,3).map(function(point){const label=[point.condition_code,point.raw_or_graded].filter(Boolean).join(" ");return (label?label+": ":"")+(point.market_price||point.mid_price||point.low_price||"")+" "+(point.currency||currency);}).filter(function(value){return value.trim()!==currency;});return (amount?amount+" "+currency:"")+(pointLabels.length?" ("+pointLabels.join("; ")+")":"");};';

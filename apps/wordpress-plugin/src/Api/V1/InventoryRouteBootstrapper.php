<?php
/**
 * Inventory route bootstrap orchestration.
 *
...
final class InventoryRouteBootstrapper {
	/**
	 * @var callable(null|list<array<string, mixed>>, array<string, mixed>): array<string, mixed>
	 */
	private $registrar;

	private InventoryRouteBootstrapStatusPresenter $presenter;

	public function __construct(
		?InventoryRouteBootstrapStatusPresenter $presenter = null,
		?callable $registrar = null
	) {
		$this->presenter = $presenter ?? new InventoryRouteBootstrapStatusPresenter();
		$this->registrar = $registrar ?? static function ( ?array $route_contracts, array $payload ): array {
			$registered_count = ( new InventoryRouteRegistrar( new InventoryRouteRegistrationPlanner() ) )
	...
	public function register(): void {
		add_action( 'rest_api_init', array( $this, 'bootstrap_current_routes' ), 22 );
	}
	...
	public function bootstrap_current_routes(): array {
		return $this->bootstrap( FeatureFlags::is_enabled( 'inventory_pricing' ) );
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function bootstrap( bool $inventory_feature_enabled, ?array $route_contracts = null ): array {
		return $this->bootstrap_from_payload(
			$this->presenter->health_payload( $inventory_feature_enabled, $route_contracts ),
			$route_contracts
		);
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function bootstrap_from_registration_args( bool $inventory_feature_enabled, array $route_plans ): array {
		return $this->bootstrap_from_payload(
			$this->presenter->health_payload_from_registration_args( $inventory_feature_enabled, $route_plans ),
			null
		);
	}

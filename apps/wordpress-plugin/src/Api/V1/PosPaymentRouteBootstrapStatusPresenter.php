<?php
/**
 * POS/payment route bootstrap status presentation.
 *
...
final class PosPaymentRouteBootstrapStatusPresenter {
	private PosPaymentRouteBootstrapPlanner $planner;

	public function __construct( ?PosPaymentRouteBootstrapPlanner $planner = null ) {
		$this->planner = $planner ?? new PosPaymentRouteBootstrapPlanner();
	}
	 ...
	 */
	public function health_payload( bool $pos_payments_feature_enabled, ?array $route_contracts = null ): array {
		return $this->payload_from_plan(
			$this->planner->plan( $pos_payments_feature_enabled, $route_contracts )
		);
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function health_payload_from_registration_args( bool $pos_payments_feature_enabled, array $route_plans ): array {
		return $this->payload_from_plan(
			$this->planner->plan_from_registration_args( $pos_payments_feature_enabled, $route_plans )
		);
	}
	 ...
	 */
	public function admin_summary( bool $pos_payments_feature_enabled, ?array $route_contracts = null ): array {
		return $this->admin_summary_from_payload(
			$this->health_payload( $pos_payments_feature_enabled, $route_contracts )
		);
	}

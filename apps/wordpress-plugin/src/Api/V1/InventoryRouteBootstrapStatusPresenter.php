<?php
/**
 * Inventory route bootstrap status presentation.
 *
...
final class InventoryRouteBootstrapStatusPresenter {
	private InventoryRouteBootstrapPlanner $planner;

	public function __construct( ?InventoryRouteBootstrapPlanner $planner = null ) {
		$this->planner = $planner ?? new InventoryRouteBootstrapPlanner();
	}
	 ...
	 */
	public function health_payload( bool $inventory_feature_enabled, ?array $route_contracts = null ): array {
		return $this->payload_from_plan(
			$this->planner->plan( $inventory_feature_enabled, $route_contracts )
		);
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function health_payload_from_registration_args( bool $inventory_feature_enabled, array $route_plans ): array {
		return $this->payload_from_plan(
			$this->planner->plan_from_registration_args( $inventory_feature_enabled, $route_plans )
		);
	}
	 ...
	 */
	public function admin_summary( bool $inventory_feature_enabled, ?array $route_contracts = null ): array {
		return $this->admin_summary_from_payload(
			$this->health_payload( $inventory_feature_enabled, $route_contracts )
		);
	}

<?php
/**
 * Gated offline route bootstrap planner.
 *
...
final class OfflineRouteBootstrapPlanner {
	private OfflineRouteRegistrationPlanner $registration_planner;

	public function __construct( ?OfflineRouteRegistrationPlanner $registration_planner = null ) {
		$this->registration_planner = $registration_planner ?? new OfflineRouteRegistrationPlanner();
	}
	 ...
	 */
	public function plan( bool $offline_feature_enabled, ?array $route_contracts = null ): array {
	 ...
	 * @return array<string, mixed>
	 */
	public function plan_from_registration_args( bool $offline_feature_enabled, array $route_plans ): array {
		$registerable_plans = array_filter(
			$route_plans,
			static fn ( array $route_plan ): bool => true === $route_plan['should_register']
		);
		$block_reasons      = $this->bootstrap_block_reasons( $offline_feature_enabled, $registerable_plans );

		return array(
			'feature_enabled'            => $offline_feature_enabled,
			'planned_route_count'        => count( $route_plans ),
			'registerable_route_count'   => count( $registerable_plans ),
			'should_register_routes'     => $offline_feature_enabled && array() !== $registerable_plans,
	 ...
	 * @return list<string>
	 */
	private function bootstrap_block_reasons( bool $offline_feature_enabled, array $registerable_plans ): array {
		$reasons = array();

		if ( ! $offline_feature_enabled ) {
			$reasons[] = 'offline_sync_feature_disabled';
		}

		if ( array() === $registerable_plans ) {
			$reasons[] = 'no_registerable_offline_routes';
		}
	...
	private function route_registration_summary( array $route_plans ): array {
		...
		foreach ( $route_plans as $route_key => $route_plan ) {
			$summary[ $route_key ] = array(
				...
				'required_scope'             => (string) $route_plan['required_scope'],
				'permission_strategy'        => (string) $route_plan['permission_strategy'],
				...
				'registration_block_reasons' => $this->list_values( $route_plan['registration_block_reasons'] ?? array() ),
			);
		}

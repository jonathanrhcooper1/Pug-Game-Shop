<?php
/**
 * POS/payment route bootstrap orchestration.
 *
...
final class PosPaymentRouteBootstrapper {
	/**
	 * @var callable(null|list<array<string, mixed>>, array<string, mixed>): array<string, mixed>
	 */
	private $registrar;

	private PosPaymentRouteBootstrapStatusPresenter $presenter;

	public function __construct(
		?PosPaymentRouteBootstrapStatusPresenter $presenter = null,
		?callable $registrar = null
	) {
		$this->presenter = $presenter ?? new PosPaymentRouteBootstrapStatusPresenter();
		$this->registrar = $registrar ?? static function ( ?array $route_contracts, array $payload ): array {
			$registered_count = ( new PosPaymentRouteRegistrar( new PosPaymentRouteRegistrationPlanner() ) )
				->register_enabled_routes( $route_contracts );
	...
	public function register(): void {
		add_action( 'rest_api_init', array( $this, 'bootstrap_current_routes' ), 21 );
	}
	...
	public function bootstrap_current_routes(): array {
		return $this->bootstrap( FeatureFlags::is_enabled( 'pos_payments' ) );
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function bootstrap( bool $pos_payments_feature_enabled, ?array $route_contracts = null ): array {
		return $this->bootstrap_from_payload(
			$this->presenter->health_payload( $pos_payments_feature_enabled, $route_contracts ),
			$route_contracts
		);
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function bootstrap_from_registration_args( bool $pos_payments_feature_enabled, array $route_plans ): array {
		return $this->bootstrap_from_payload(
			$this->presenter->health_payload_from_registration_args( $pos_payments_feature_enabled, $route_plans ),
			null
		);
	}

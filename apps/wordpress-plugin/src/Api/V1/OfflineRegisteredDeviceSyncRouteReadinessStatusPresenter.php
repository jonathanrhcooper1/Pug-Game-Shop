...
final class OfflineRegisteredDeviceSyncRouteReadinessStatusPresenter {
	...
	public function admin_summary(): array {
		...
		return array(
			'value'  => sprintf(
				'pull %s; push %s; context %s; query plan %s; SQL plan %s; repository %s; provider %s; route provider %s; cursor planner %s; cursor SQL %s; cursor repository %s; %d callbacks; writes deferred; routes deferred',

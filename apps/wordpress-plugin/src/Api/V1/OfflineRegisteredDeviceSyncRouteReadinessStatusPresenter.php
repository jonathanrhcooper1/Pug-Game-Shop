...
final class OfflineRegisteredDeviceSyncRouteReadinessStatusPresenter {
	...
	public function admin_summary(): array {
		...
		return array(
			'value'  => sprintf(
				'pull %s; push %s; context %s; query plan %s; SQL plan %s; repository %s; provider %s; route provider %s; cursor planner %s; cursor SQL %s; cursor repository %s; route cursor provider %s; handler cursor %s; handler factory %s; route dependencies %s; push snapshots %s; push snapshot SQL %s; push snapshot repository %s; push persistence %s; push SQL %s; push repository %s; push route handler %s; push route provider %s; push handler factory %s; push route dependencies %s; %d callbacks; writes deferred; routes deferred',

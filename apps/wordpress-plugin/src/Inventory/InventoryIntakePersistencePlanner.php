...
final class InventoryIntakePersistencePlanner {
	...
	private function insert_row(
		...
		return array(
			...
			'status'                        => $status,
			'woocommerce_product_id'        => $this->nullable_positive_int( $item['woocommerce_product_id'] ?? null ),
			'square_catalog_item_id'        => $this->nullable_string( $item['square_catalog_item_id'] ?? null ),
			'square_catalog_variation_id'   => $this->nullable_string( $item['square_catalog_variation_id'] ?? null ),
			'square_location_id'            => $this->nullable_string( $item['square_location_id'] ?? null ),
			'external_sync_state'           => ! empty( $item['square_catalog_variation_id'] ) ? 'square_synced' : 'pending',

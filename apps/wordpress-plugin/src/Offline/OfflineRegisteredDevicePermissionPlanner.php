	...
	public function plan(
		...
		if ( ! $lookup_plan->is_valid() ) {
			return OfflineRegisteredDevicePermissionPlan::denied(
				$lookup_plan,
				$lookup_plan->errors(),
				$this->audit_payload( 'token_lookup_rejected', $required_scope, $lookup_plan, null, null, $lookup_plan->errors() )
			);
		}
		...
		if ( null === $device_row ) {
			return OfflineRegisteredDevicePermissionPlan::lookup_required(
				$lookup_plan,
				$this->audit_payload( 'device_lookup_required', $required_scope, $lookup_plan, null, null, array() )
			);
		}
		...
		if ( ! $decision->is_allowed() ) {
			return OfflineRegisteredDevicePermissionPlan::denied(
				$lookup_plan,
				$decision->errors(),
				$this->audit_payload( 'device_authorization_denied', $required_scope, $lookup_plan, $decision, null, $decision->errors() ),
				$decision
			);
		}
		...
		try {
			...
			return OfflineRegisteredDevicePermissionPlan::denied(
				$lookup_plan,
				$errors,
				$this->audit_payload( 'device_session_rejected', $required_scope, $lookup_plan, $decision, null, $errors ),
				$decision
			);
		}
		...
		return OfflineRegisteredDevicePermissionPlan::authorized(
			...
			$decision,
			$session_plan,
			$this->audit_payload( 'authorized', $required_scope, $lookup_plan, $decision, $session_plan, array() )
		);
	}

...
assert.equal(contract.authorities.global_source_of_truth, "local_sync_server")
...
assert.equal(contract.authorities.store_credit_authority, "local_sync_server_credit_ledger")
assert.equal(contract.authorities.wordpress_inventory_pull_authority, "manual_remote_authority_only")
assert.equal(contract.authorities.scrydex_reference_catalog_authority, "local_sync_server_after_provider_refresh")
assert.equal(contract.authorities.scrydex_webhook_authority, "wordpress_signed_receiver_to_lan_relay")
...
assert.equal(contract.safety.square_count_polling_mutates_inventory_by_default, false)
assert.equal(contract.safety.wordpress_inventory_pull_mutates_inventory_by_default, false)
assert.equal(contract.safety.wordpress_inventory_pull_authority, "manual_remote_authority_only")
...
assert.equal(contract.safety.scrydex_webhook_relay_token_required, true)
assert.equal(contract.safety.scrydex_webhook_refresh_processes_all_expansion_ids, true)
assert.equal(contract.safety.scrydex_daily_sync_runs_on_middleman, true)
...
assert.ok(localSyncStoreSource.includes("websitePulledInventoryQuantityOnHand("))
assert.ok(localSyncStoreSource.includes("wordpressInventoryPullAllowsQuantityIncrease("))
assert.ok(localSyncStoreSource.includes("shouldConsolidateInventoryQuantityGroup(input)"))
...
assert.ok(contract.responsibilities.includes("keep_scrydex_credentials_server_side_only"))
assert.ok(contract.responsibilities.includes("accept_scrydex_expansion_price_webhook_relay_from_wordpress"))
assert.ok(contract.responsibilities.includes("process_all_scrydex_webhook_expansion_ids"))
assert.ok(contract.responsibilities.includes("run_daily_scrydex_catalog_price_sync_from_middleman"))
assert.ok(contract.responsibilities.includes("reprice_inventory_from_local_reference_catalog_with_floor_protection"))
...
for (const endpoint of [
  ...
  "POST /pos/square/terminal/receipt",
  "GET /pos/cash-drawer/status",
  "POST /pos/cash-drawer/kick",
  "POST /kiosk/orders",
  ...
  "POST /trade-ins/orders/:order_id/void",
  "GET /kiosk/orders",

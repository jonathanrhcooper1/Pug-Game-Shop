...
const server = createLocalSyncHttpServer({
  ...
  storeOptions: {
    ...
    squareTerminalConnector: {
      ...
      retrieveCheckoutReceipt: async (input) => {
        receiptLookupCall = input
        ...
        return {
          ...
          square_checkout: {
            ...
            created_at: "2026-06-15T20:00:00.000Z",
            updated_at: "2026-06-15T20:01:00.000Z",
          },
          payment: {
            id: "payment-1",
            status: "COMPLETED",
            ...
            created_at: "2026-06-15T20:00:30.000Z",
            updated_at: "2026-06-15T20:01:00.000Z",
            amount_money: { amount: 2599, currency: "USD" },
            total_money: { amount: 2599, currency: "USD" },
            approved_money: { amount: 2599, currency: "USD" },
            card_details: {
              status: "CAPTURED",
              card_brand: "VISA",
              last_4: "1111",
          ...
          provider_request_performed: true,
          payment_capture_completed: true,
          credentials_synced_to_client: false,
          raw_credentials_returned: false,
        }
      },
...
try {
  ...
  const receipt = await fetchJson(`${baseUrl}/pos/square/terminal/receipt`, {
    method: "POST",
    headers: authHeaders(token),
    body: {
      checkout_id: checkout.square_checkout.id,
    },
  })
  assert.equal(receipt.status, "ok")
  assert.equal(receipt.receipt_ready, true)
  assert.equal(receipt.receipt_number, "RCP-123")
  assert.equal(receipt.payment_id, "payment-1")
  assert.equal(receipt.raw_credentials_returned, false)
  assert.equal(receiptLookupCall.checkout_id, "terminal-checkout-1")

...
try {
  const normalPull = await store.pullWebsiteInventoryForSystem({
    domains: ["inventory"],
    page: 1,
    page_size: 100,
  ...
  assert.equal(normalPull.status, "ok")
  assert.equal(normalPull.inserted_count, 0)
  assert.equal(normalPull.wordpress_inventory_pull_skipped_remote_authority, true)
  assert.equal(normalPull.inventory_source_of_truth, "local_sync_server")
  assert.equal(normalPull.wordpress_inventory_pull_authority, "manual_remote_authority_only")
  assert.equal(pullInputs.length, 0)
  ...
  assert.equal(firstPull.wordpress_inventory_remote_authority, true)
  ...
  assert.equal(quantityChangePull.updated_count, 0)
  assert.equal(quantityChangePull.wordpress_inventory_pull_skipped_remote_authority, true)
  assert.equal(quantityChangePull.square_catalog_inventory_sync_accepted_count, 0)
  assert.equal(squareCalls.length, 1)

  const manualQuantityChangePull = await store.pullWebsiteInventoryForSystem({
    domains: ["inventory"],
    page: 1,
    page_size: 100,
    updated_after: "2026-06-25T12:03:00Z",
    inventory_authority: "wordpress",
  })

  assert.equal(manualQuantityChangePull.status, "ok")
  assert.equal(manualQuantityChangePull.updated_count, 1)
  assert.equal(manualQuantityChangePull.square_catalog_inventory_sync_accepted_count, 1)
  ...
  pullRows = [wordpressInventoryRow({ quantity_on_hand: 4, row_version: 3, sale_price: "6.00" })]
  const ignoredIncreasePull = await store.pullWebsiteInventoryForSystem({
    domains: ["inventory"],
    page: 1,
    page_size: 100,
    updated_after: "2026-06-25T12:04:00Z",
    manual_remote_authority: true,
  })

  assert.equal(ignoredIncreasePull.status, "ok")
  assert.equal(ignoredIncreasePull.unchanged_count, 1)
  assert.equal(ignoredIncreasePull.remote_quantity_increase_ignored_count, 1)
  assert.equal(ignoredIncreasePull.square_catalog_inventory_sync_accepted_count, 0)
  assert.equal(squareCalls.length, 2)

  const allowedIncreasePull = await store.pullWebsiteInventoryForSystem({
    domains: ["inventory"],
    page: 1,
    page_size: 100,
    updated_after: "2026-06-25T12:05:00Z",
    manual_remote_authority: true,
    allow_remote_inventory_increase: true,
  })

  assert.equal(allowedIncreasePull.status, "ok")
  assert.equal(allowedIncreasePull.updated_count, 1)
  assert.equal(allowedIncreasePull.remote_quantity_increase_ignored_count, 0)
  assert.equal(allowedIncreasePull.square_catalog_inventory_sync_accepted_count, 1)
  assert.equal(squareCalls.length, 3)
  assert.equal(squareCalls[2].quantity_on_hand, 4)
  ...
  assert.equal(status.last_website_inventory_pull.remote_quantity_increase_ignored_count, 0)
  assert.equal(status.inventory_source_of_truth, "local_sync_server")
  assert.equal(status.wordpress_inventory_pull_authority, "manual_remote_authority_only")

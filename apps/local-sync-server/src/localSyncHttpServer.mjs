...
import { getCashDrawerStatus, kickCashDrawer } from "./cashDrawerKick.mjs"
...
export function createLocalSyncHttpServer(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/scrydex/catalog/status") {
        return sendStoreResult(response, store.getScryDexCatalogStatus(token))
      }

      if (request.method === "POST" && url.pathname === "/scrydex/catalog/sync-jobs") {
        return sendStoreResult(response, store.startScryDexCatalogSyncJob(token, await readJson(request)))
      }

      if (request.method === "POST" && url.pathname === "/scrydex/catalog/webhook-refresh") {
      ...
      if (request.method === "POST" && url.pathname === "/pos/square/terminal/receipt") {
        return sendStoreResult(response, await store.retrieveSquareTerminalReceipt(token, await readJson(request)))
      }

      if (request.method === "GET" && url.pathname === "/pos/cash-drawer/status") {
        const access = store.authorizeCashDrawer(token)
        ...
        return sendStoreResult(
          response,
          await getCashDrawerStatus(
            { printer_name: url.searchParams.get("printer_name") ?? "" },
            {
      ...
      if (request.method === "POST" && url.pathname === "/pos/cash-drawer/kick") {
        const access = store.authorizeCashDrawer(token)
        ...
        return sendStoreResult(
          response,
          await kickCashDrawer(await readJson(request), {
            requestedByUserId: access.user_id,
            requestedByUserName: access.user_name,
          }),
        )
      ...
      if (request.method === "GET" && url.pathname === "/pos/cash-drawer/session") {
        return sendStoreResult(response, store.getCashDrawerSession(token))
      }

      if (request.method === "POST" && url.pathname === "/pos/cash-drawer/session/open") {
        return sendStoreResult(response, store.openCashDrawerSession(token, await readJson(request)))
      }

      if (request.method === "POST" && url.pathname === "/pos/cash-drawer/session/add-cash") {
        return sendStoreResult(response, store.addCashToDrawer(token, await readJson(request)))
      }

      if (request.method === "POST" && url.pathname === "/pos/cash-drawer/session/remove-cash") {
        return sendStoreResult(response, store.removeCashFromDrawer(token, await readJson(request)))
      }

      if (request.method === "POST" && url.pathname === "/pos/cash-drawer/session/close") {
        return sendStoreResult(response, store.closeCashDrawerSession(token, await readJson(request)))
      }

      if (request.method === "POST" && url.pathname === "/pos/cash-drawer/payout-check") {
        return sendStoreResult(response, store.checkCashDrawerPayout(token, await readJson(request)))
      }
      ...
      const tradeInVoidMatch = url.pathname.match(/^\/trade-ins\/orders\/([^/]+)\/void$/)

      if (request.method === "POST" && tradeInVoidMatch) {
        return sendStoreResult(
          response,
          store.voidTradeInOrder(token, decodeURIComponent(tradeInVoidMatch[1]), await readJson(request)),
        )
      }
      ...
      if (request.method === "POST" && url.pathname === "/server/maintenance/square-sync-local-inventory") {
        return sendStoreResult(response, await store.syncLocalInventoryToSquare(token, await readJson(request)))
      }

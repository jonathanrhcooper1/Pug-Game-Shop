...
export function createSquareTerminalConnector(options = {}) {
  ...
  async function createDeviceCode(input = {}) {
    ...
    const result = await squarePost("/v2/devices/codes", body)
  ...
  async function createCheckout(input = {}) {
    ...
    const result = await squarePost("/v2/terminals/checkouts", body)
  ...
  async function retrieveCheckoutReceipt(input = {}) {
    const currentStatus = status()
    ...
    const checkoutId = cleanExternalId(input.checkout_id ?? input.checkoutId ?? input.terminal_checkout_id ?? input.terminalCheckoutId)

    if (!checkoutId) {
      return blocked("square_terminal_checkout_id_required", "Send the payment to Square first, then retrieve the receipt.")
    }

    const checkoutResult = await squareGet(`/v2/terminals/checkouts/${encodeURIComponent(checkoutId)}`)

    if (checkoutResult.status !== "ok") {
      return checkoutResult
    }

    const checkout = publicTerminalCheckout(checkoutResult.payload?.checkout)
    const paymentId = checkout.payment_ids[0] ?? ""

    if (checkout.status !== "COMPLETED" || !paymentId) {
      return {
        status: "ok",
        action: "square_terminal_receipt_retrieved",
        square_checkout: checkout,
        receipt_ready: false,
        receipt_number: "",
        receipt_url: "",
        ...
        payment_status: "",
        provider_request_performed: true,
        payment_capture_completed: false,
        credentials_synced_to_client: false,
        raw_credentials_returned: false,
      }
    }

    const paymentResult = await squareGet(`/v2/payments/${encodeURIComponent(paymentId)}`)

    if (paymentResult.status !== "ok") {
      return paymentResult
    }

    const payment = publicPayment(paymentResult.payload?.payment)
    ...
    return {
      ...
      square_checkout: checkout,
      payment,
      receipt_ready: payment.status === "COMPLETED",
      receipt_number: payment.receipt_number || payment.id,
      receipt_url: payment.receipt_url,
      payment_id: payment.id,
      payment_status: payment.status,
      provider_request_performed: true,
      payment_capture_completed: payment.status === "COMPLETED",
      credentials_synced_to_client: false,
      raw_credentials_returned: false,
    }
  }
  ...
  async function squarePost(path, body) {
    return squareRequest("POST", path, body)
  }

  async function squareGet(path) {
    return squareRequest("GET", path)
  }

  async function squareRequest(method, path, body = undefined) {
    let response
    let payload
...
function publicPayment(payment = {}) {
  return {
    id: cleanExternalId(payment.id),
    status: cleanExternalId(payment.status),
    receipt_number: cleanExternalId(payment.receipt_number),
    receipt_url: cleanHttpUrl(payment.receipt_url),
    created_at: cleanTimestamp(payment.created_at),
    updated_at: cleanTimestamp(payment.updated_at),
    amount_money: publicMoney(payment.amount_money),
    total_money: publicMoney(payment.total_money),
    approved_money: publicMoney(payment.approved_money),
    card_details: {
      status: cleanExternalId(payment.card_details?.status),
      card_brand: cleanExternalId(payment.card_details?.card?.card_brand),
      last_4: cleanExternalId(payment.card_details?.card?.last_4).slice(0, 4),
      entry_method: cleanExternalId(payment.card_details?.entry_method),
    },
  }
}
...
function publicMoney(value = {}) {
  return {
    amount: Math.max(0, Number.parseInt(String(value?.amount ?? 0), 10)),

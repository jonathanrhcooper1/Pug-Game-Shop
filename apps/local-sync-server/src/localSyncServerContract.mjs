...
export const LOCAL_SYNC_SERVER_ENDPOINTS = Object.freeze([
  ...
  { method: "POST", path: "/pos/square/terminal/receipt", purpose: "Staff Square Terminal checkout receipt lookup from completed payment" },
  { method: "GET", path: "/pos/cash-drawer/status", purpose: "Staff receipt-printer cash drawer readiness check" },
  { method: "POST", path: "/pos/cash-drawer/kick", purpose: "Staff receipt-printer cash drawer kick for cash and split checkout" },
  { method: "GET", path: "/pos/cash-drawer/session", purpose: "Staff cash drawer shift status, movements, and closeout summary" },
  { method: "POST", path: "/pos/cash-drawer/session/open", purpose: "Staff cash drawer shift open with starting bank amount" },
  { method: "POST", path: "/pos/cash-drawer/session/add-cash", purpose: "Staff cash drawer bank cash add for bank runs and manager adjustments" },
  { method: "POST", path: "/pos/cash-drawer/session/remove-cash", purpose: "Staff cash drawer bank cash remove for deposits and manager adjustments" },
  { method: "POST", path: "/pos/cash-drawer/session/close", purpose: "Staff end-of-day drawer count, variance, and reconciliation report" },
  { method: "POST", path: "/pos/cash-drawer/payout-check", purpose: "Trade-in cash payout warning when requested cash exceeds drawer cash" },
  ...
  { method: "PATCH", path: "/inventory/items/:inventory_public_id", purpose: "Staff inventory quantity, price, floor, visibility, and location update" },
  ...
  { method: "POST", path: "/trade-ins/orders/:order_id/void", purpose: "Manager trade-in void and local store-credit reversal" },
  ...
  { method: "POST", path: "/server/maintenance/website-pull", purpose: "Manager-triggered website inventory/catalog/event pull into LAN cache" },
  { method: "POST", path: "/server/maintenance/square-sync-local-inventory", purpose: "Manager-triggered push of local authoritative inventory counts to Square" },
  { method: "POST", path: "/server/maintenance/patch", purpose: "Manager-staged LAN server package patch with checksum validation" },
  { method: "POST", path: "/server/maintenance/restart", purpose: "Manager-scheduled LAN server restart using installed startup script" },
...
export function buildLocalSyncServerContract(options = {}) {
  ...
  return {
    ...
    authorities: {
      global_source_of_truth: "local_sync_server",
      local_offline_authority: "local_sync_server",
      client_authority: "none_clients_request_locks",
      payment_authority: "local_square_pos",
      store_credit_authority: "local_sync_server_credit_ledger",
      wordpress_inventory_pull_authority: "manual_remote_authority_only",
      scrydex_reference_catalog_authority: "local_sync_server_after_provider_refresh",
      scrydex_webhook_authority: "wordpress_signed_receiver_to_lan_relay",
    },
    ...
    responsibilities: [
      ...
      "serve_pricecharting_primary_graded_prices_with_scrydex_reference_fallback",
      ...
      "pull_non_inventory_changes_from_wordpress_when_online",
      "keep_wordpress_inventory_pull_manual_remote_authority_only",
      "push_absolute_local_inventory_counts_to_square",
      "push_local_operations_to_wordpress_when_online",
      "keep_scrydex_credentials_server_side_only",
      "accept_scrydex_expansion_price_webhook_relay_from_wordpress",
      "process_all_scrydex_webhook_expansion_ids",
      "run_daily_scrydex_catalog_price_sync_from_middleman",
      "reprice_inventory_from_local_reference_catalog_with_floor_protection",
    ...
    safety: {
      ...
      square_count_polling_mutates_inventory_by_default: false,
      wordpress_inventory_pull_mutates_inventory_by_default: false,
      wordpress_inventory_pull_authority: "manual_remote_authority_only",
      ...
      scrydex_webhook_relay_token_required: true,
      scrydex_webhook_refresh_processes_all_expansion_ids: true,
      scrydex_daily_sync_runs_on_middleman: true,
      ...
      graded_pricing_pricecharting_primary: true,
      graded_pricing_scrydex_fallback: true,
    },
  }
}

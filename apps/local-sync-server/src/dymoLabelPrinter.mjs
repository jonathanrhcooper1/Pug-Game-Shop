...
export async function printDymoInventoryLabel(input = {}, options = {}) {
  ...
  if (label.scan_code.length > SCANNER_BARCODE_MAX_LENGTH) {
    return blocked(
      "barcode_too_long_for_scanner",
      `Barcode ${label.scan_code} is ${label.scan_code.length} characters. Scanner-safe labels must be ${SCANNER_BARCODE_MAX_LENGTH} characters or less.`,
      {
        action: "dymo_label_print",
        service_base_url: serviceBaseUrl,
        scan_code: label.scan_code,
        direct_print_performed: false,
        credentials_synced_to_client: false,
        raw_credentials_returned: false,
      },
    )

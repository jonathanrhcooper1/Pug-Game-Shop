...
export function createSquareCatalogInventorySyncer(options = {}) {
  ...
  async function syncInventoryItem({ operation, item } = {}) {
    ...
    return {
      ...
      category_sync_status: catalog.category_sync_status ?? "",
  ...
  async function createCatalogItemVariation(plan) {
    ...
    body.idempotency_key = squareIdempotencyKey(plan.idempotency_key, "catalog:create", body)
  ...
  async function upsertExistingVariation(plan) {
    ...
    body.idempotency_key = squareIdempotencyKey(plan.idempotency_key, "catalog:update:variation-only", body)
    const result = await squareRequest("/v2/catalog/batch-upsert", { method: "POST", body })
    ...
    return {
      ...
      category_sync_status: "existing_item_skipped",
      item_created: false,
  ...
  async function maybeAttachCatalogImage(plan) {
    ...
    return createCatalogImage({
      square_catalog_item_id: plan.square_catalog_item_id,
      idempotency_key: squareIdempotencyKey(plan.idempotency_key, "image", {
        square_catalog_item_id: plan.square_catalog_item_id,
  ...
  async function retrieveVariation(variationId) {
    ...
    const itemId = cleanExternalId(object.item_variation_data?.item_id)
    const relatedObjects = Array.isArray(result.payload?.related_objects) ? result.payload.related_objects : []
    const relatedItem = relatedObjects.find(
      (candidate) => candidate?.type === "ITEM" && cleanExternalId(candidate?.id) === itemId,
    )
  ...
  async function retrieveCatalogItem(itemId) {
    const safeItemId = cleanExternalId(itemId)
    ...
    const result = await squareRequest(
      `/v2/catalog/object/${encodeURIComponent(safeItemId)}`,
      { method: "GET" },
      { allowNotFound: true },
    )
    ...
    return {
      status: "ok",
      square_catalog_item_id: cleanExternalId(object.id),
      item_version: positiveInt(object.version),
    }
  }
      ...
      changes: [
        {
          ...
          physical_count: {
            ...
            state: "IN_STOCK",
            occurred_at: plan.inventory_occurred_at_utc,
          },
        },
      ],
    }
    body.idempotency_key = squareIdempotencyKey(plan.idempotency_key, "inventory", body)
...
function buildSquareSyncPlan(operation, item, locationId) {
  ...
  const variationName = cleanText(
    [rawOrGraded === "sealed" ? "Sealed" : gradeLabel || condition, cleanText(item?.variant), cleanText(item?.finish)]
      .filter(Boolean)
      .join(" / "),
  ) || "Default"
  ...
  const kioskVisibility = cleanVisibility(item?.kiosk_visibility)
  const squareQuantity = status === "available" && kioskVisibility === "visible" ? quantityOnHand : 0
  ...
  if (kioskVisibility !== "visible" && !squareCatalogItemId && !squareCatalogVariationId) {
    return skipped(
      "square_kiosk_visibility_hidden",
      "This inventory row is hidden from kiosk and has no existing Square mapping, so Square sync was skipped.",
    )
  }
  ...
  return {
    ...
    description: cleanText(
      [
        ...
        rawOrGraded === "graded" ? gradeLabel : rawOrGraded === "sealed" ? "Sealed product" : condition,
    ...
    inventory_occurred_at_utc: inventoryOccurredAtUtc,
...
function squareIdempotencyKey(seed, scope, payload = {}) {
  const cleanSeed = cleanIdempotencyKey(seed) || "pug-square-sync"
  const cleanScope = cleanIdempotencyKey(scope) || "request"
  const hash = createHash("sha256")
    .update(stableJsonStringify(stripIdempotencyKey(payload)))
    .digest("hex")
    .slice(0, 16)
  return cleanIdempotencyKey(`${cleanSeed}:${cleanScope}:${hash}`).slice(0, 128)
}

function stripIdempotencyKey(value) {
  if (Array.isArray(value)) {
    return value.map(stripIdempotencyKey)
  }
  ...
  return Object.fromEntries(
    Object.entries(value)
      ...
      .map(([key, entry]) => [key, stripIdempotencyKey(entry)]),
  )
}
...
function squareCategoryHierarchy(plan) {
  const sealedCategoryText = `${plan.item_name ?? ""} ${plan.description ?? ""}`.toLowerCase()
  const root =
    plan.raw_or_graded === "graded"
      ? "Graded"
      : plan.raw_or_graded === "sealed"
        ? sealedCategoryText.includes("accessor")
          ? "Accessories"
          : "Sealed"
        : "Singles"
...
function cleanRawOrGraded(value) {
  const text = String(value ?? "").trim().toLowerCase()
  return ["graded", "sealed"].includes(text) ? text : "raw"
}
...
function squareAbbreviation(game, rawOrGraded) {
  ...
  return `${prefix}${rawOrGraded === "graded" ? "G" : rawOrGraded === "sealed" ? "S" : ""}`.slice(0, 24)
}

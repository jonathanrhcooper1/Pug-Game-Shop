...
import { createScryDexCatalogIndexer } from "./scrydexCatalogIndexer.mjs"
...
const wordpressInventoryPollDisabled = true || envFlag(
...
const scryDexWebhookRelayToken = firstEnv(
...
const localScryDexCatalogIndexer = createScryDexCatalogIndexer({
  apiKey: firstEnv("SCRYDEX_API_KEY", "PUG_SCRYDEX_API_KEY", "SCRYDEX_VISION_API_KEY", "PUG_SCRYDEX_VISION_API_KEY"),
  teamId: firstEnv("SCRYDEX_TEAM_ID", "PUG_SCRYDEX_TEAM_ID", "SCRYDEX_VISION_TEAM_ID", "PUG_SCRYDEX_VISION_TEAM_ID"),
  baseUrl: firstEnv("SCRYDEX_BASE_URL", "PUG_SCRYDEX_BASE_URL"),
...
const server = await listenLocalSyncHttpServer({
  ...
  storeOptions: {
    ...
    wordpressCatalogIndexer: localScryDexCatalogIndexer ?? wordpressCatalogIndexer,
    scryDexCatalogIndexerMode: localScryDexCatalogIndexer ? "local_scrydex_api" : "wordpress_route",
    wordpressCatalogIndexerFallback: wordpressCatalogIndexer,
...
if (!scryDexDailySyncDisabled && (localScryDexCatalogIndexer || wordpressCatalogIndexer)) {
...
function startWordPressInventoryPolling(server, pollSeconds, maxPages) {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (!bootstrapComplete) {
        console.log(`WordPress manual remote-authority inventory pass paused after ${pagesProcessed} page(s); continuing at page ${bootstrapPage}.`)
      } else if (mode === "bootstrap") {
        console.log("WordPress manual remote-authority inventory pass complete.")
      }
    } catch (error) {
  ...
  server.once("close", () => clearInterval(interval))
  console.log(
    `WordPress manual remote-authority inventory polling enabled every ${pollSeconds}s, up to ${maxPages} page(s) per cycle.`,
  )
  void run()
}
...
function startScryDexDailyCatalogSync(server, options) {
  ...
  const runIfDue = async () => {
    ...
    const runKey = localDateKey(current)
    const currentHour = current.getHours()
    const shouldRunScheduledHour = currentHour === options.hour
    const shouldCatchUpMissedWindow =
      currentHour > options.hour &&
      lastRunKey !== runKey &&
      !scryDexCatalogFreshForLocalDate(store, runKey)

    if ((!shouldRunScheduledHour && !shouldCatchUpMissedWindow) || lastRunKey === runKey) {
      return
    }
...
function scryDexCatalogFreshForLocalDate(store, runKey) {
  if (!store || typeof store.getScryDexCatalogStatusForSystem !== "function") {
    return false
  }

  try {
    const status = store.getScryDexCatalogStatusForSystem()
    const latestCatalogSyncUtc = status?.totals?.latest_catalog_sync_utc
    const latestPriceObservedUtc = status?.totals?.latest_price_observed_utc

    return [latestCatalogSyncUtc, latestPriceObservedUtc]
      .map((value) => {
        const date = new Date(value)
        return Number.isNaN(date.getTime()) ? "" : localDateKey(date)
      })
      .some((dateKey) => dateKey === runKey)
  } catch {
    return false
  }
}

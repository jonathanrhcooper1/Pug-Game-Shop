...
The app now applies sanitized inventory rows and the active customer credit
account from successful live desktop pull responses into the local cache, but
event cache mutation, full customer-directory cache mutation, conflict
resolution replay, printer/scanner adapters, kiosk lockdown, and signed updater
behavior still need follow-on passes. Browser mode still previews queue
persistence, while the desktop Tauri command now writes accepted operations to
local SQLite and can run guarded authenticated pull/push requests when a paired
device token exists.

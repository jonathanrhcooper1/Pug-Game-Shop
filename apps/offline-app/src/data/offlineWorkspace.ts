  ...
  eventSnapshots: [
    {
      ...
      startsAtUtc: "2026-07-12T23:00:00Z",
      startsAtLabel: "Sun Jul 12, 7:00 PM",
      eventType: "commander",
      game: "magicthegathering",
      entryFeeMinorUnits: 1000,
      registrationDeadlineUtc: "2026-07-12T21:00:00Z",
    ...
    },
    {
      ...
      startsAtUtc: "2026-07-14T17:00:00Z",
      startsAtLabel: "Tue Jul 14, 1:00 PM",
      eventType: "league",
      game: "pokemon",
      entryFeeMinorUnits: 500,
      registrationDeadlineUtc: "2026-07-14T16:00:00Z",
      woocommerceProductId: 0,
...
export function formatMoney(minorUnits: number, currency: string = "USD") {
...
export function signedMoneyInputFromMinorUnits(minorUnits: number) {
  const roundedMinorUnits = Number.isFinite(minorUnits) ? Math.round(minorUnits) : 0
  const prefix = roundedMinorUnits < 0 ? "-" : ""

  return `${prefix}${(Math.abs(roundedMinorUnits) / 100).toFixed(2)}`
}

export function moneyInputDraftWithTwoDecimals(value: string) {
  const normalized = value.trim().replace(/^\$/, "").replaceAll(",", "")
  const negativePrefix = normalized.startsWith("-") ? "-" : ""
  ...
  if (!hasDecimal) {
    return `${negativePrefix}${whole}`
  }

  return `${negativePrefix}${whole || "0"}.${decimalParts.join("").slice(0, 2)}`
}
...
export function buildInventoryUpdateOperation(
  ...
  options: {
    ...
    priceMinorUnits?: number
    minimumSalePriceMinorUnits?: number
    onlineVisibility?: InventoryItem["onlineVisibility"]
    kioskVisibility?: InventoryItem["kioskVisibility"]
    posVisibility?: InventoryItem["posVisibility"]
  ...
  return {
    ...
    payload_json: JSON.stringify({
      ...
      price_minor_units: options.priceMinorUnits ?? item.priceMinorUnits,
      minimum_sale_price_minor_units:
        options.minimumSalePriceMinorUnits ?? item.minimumSalePriceMinorUnits ?? item.priceMinorUnits,
      status: item.status,
      online_visibility: options.onlineVisibility ?? item.onlineVisibility ?? "visible",
      kiosk_visibility: options.kioskVisibility ?? item.kioskVisibility ?? "visible",
      pos_visibility: options.posVisibility ?? item.posVisibility ?? "visible",
      ...
      ...(typeof options.quantityOnHand === "number" ? { quantity_on_hand: options.quantityOnHand } : {}),
